...
for i in todo_list.get('data').get('list'):
    ...
    if procDefName == '数据获取':
        ...
        if '10.246.2.160' in db_name or '10.246.2.160' in db_url:
            ...
            db_type = 'mysql'
            fileInfo = '您的数据已导出，n请登录rrswl导数ftp，打开文件资源管理器（任意文件夹），输入地址 ftp://10.135.30.96/ 输入 用户名:omsexp 密码:7wS&$M7ffGfLdg93，查找文件：'
        elif 'mycat_oms_otm_exp' in db_name or 'odsotm' in db_url:
            db = 'mycat_oms_otm_exp'
            db_type = 'mysql'
            fileInfo = '您的数据已导出，n请登录rrswl导数ftp，打开文件资源管理器（任意文件夹），输入地址 ftp://10.135.30.96/ 输入 用户名:omsexp 密码:7wS&$M7ffGfLdg93，查找文件：'
        elif 'mysql_oms_cus_st_exp' in db_name:
            db = 'mysql_oms_cus_st_exp'
            db_type = 'mysql'
            fileInfo = '您的数据已导出，n请登录rrswl导数ftp，打开文件资源管理器（任意文件夹），输入地址 ftp://10.135.30.96/ 输入 用户名:omsexp 密码:7wS&$M7ffGfLdg93，查找文件：'
            ...
            db_type = 'oracle'
            fileInfo = '您的数据已导出，n请登录rrswl导数ftp，打开文件资源管理器（任意文件夹），输入地址 ftp://10.135.30.96/ 输入 用户名:omsexp 密码:7wS&$M7ffGfLdg93，查找文件：'
        elif 'mysql_wlpz_beiye_dms_exp' in db_name:
            db = 'mysql_wlpz_beiye_dms_exp'
            db_type = 'mysql'
            fileInfo = '您的数据已导出，n请登录rrswl导数ftp，打开文件资源管理器（任意文件夹），输入地址 ftp://10.135.30.96/ 输入 用户名:omsexp 密码:7wS&$M7ffGfLdg93，查找文件：'
            ...
            db_type = 'mysql'
            fileInfo = '您的数据已导出，n请登录rrswl导数ftp，打开文件资源管理器（任意文件夹），输入地址 ftp://10.135.30.96/ 输入 用户名:omsexp 密码:7wS&$M7ffGfLdg93，查找文件：'
        elif 'srm_exp' in db_name:
            db = 'srm_exp'
            db_type = 'mysql'
            fileInfo = '您的数据已导出，n请登录rrswl导数ftp，打开文件资源管理器（任意文件夹），输入地址 ftp://10.135.30.96/ 输入 用户名:omsexp 密码:7wS&$M7ffGfLdg93，查找文件：'
        elif 'newoms_mdm_exp' in db_name:
            db = 'newoms_mdm_exp'
            db_type = 'mysql'
            fileInfo = '您的数据已导出，n请登录rrswl导数ftp，打开文件资源管理器（任意文件夹），输入地址 ftp://10.135.30.96/ 输入 用户名:omsexp 密码:7wS&$M7ffGfLdg93，查找文件：'
        ...
        elif 'mysql_lejia_read_exp' in db_name:
            db = 'mysql_lejia_read_exp'
            db_type = 'mysql'
            fileInfo = '您的数据已导出，n请登录rrswl导数ftp，打开文件资源管理器（任意文件夹），输入地址 ftp://10.135.30.96/ 输入 用户名:cdkexp 密码:8UytK6Z*toY5，查找文件：'
        ...
        elif 'clgjdb' in db_url:
            db = 'gps_exp'
            db_type = 'oracle'
            fileInfo = '您的数据已导出，n请登录rrswl导数ftp，打开文件资源管理器（任意文件夹），输入地址 ftp://10.135.30.96/ 输入 用户名:tmsexp 密码:dSLLg6YLYbRBaFoU，查找文件：'
        elif 'iwmsdb_exp' in db_name or 'iwms_sysdb_exp' in db_name or '10.246.82.104' in db_url:
            db = 'iwmsdb_exp'
            db_type = 'mysql'
            fileInfo = '您的数据已导出，n请登录rrswl导数ftp，打开文件资源管理器（任意文件夹），输入地址 ftp://10.135.30.96/ 输入 用户名:iwmsexp 密码:h$6m$LzBZESvwTcr，查找文件：'
        elif 'ldg' in db_name:
        ...
        elif 'oracle_bms_exp' in db_name:
            db = 'oracle_bms_exp'
            db_type = 'oracle'
            fileInfo = '您的数据已导出，n请登录rrswl导数ftp，打开文件资源管理器（任意文件夹），输入地址 ftp://10.135.30.96/ 输入 用户名:bmsexp 密码:cvh$UhTqVk*eYfHr，查找文件：'
        elif 'mysql_kx_exp' in db_name:
            db = 'mysql_kx_exp'
            db_type = 'mysql'
